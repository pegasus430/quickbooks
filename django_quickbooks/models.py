...
class QBDTaskMixin(models.Model):
    ...
    def get_request(self, realm_id = None):
        ...
        if self.qb_operation == QUICKBOOKS_ENUMS.OPP_QR:
            return service.all()
        elif self.qb_operation == QUICKBOOKS_ENUMS.OPP_QR_BY_DATE:
            
            realm = Realm.objects.filter(id=realm_id).first()
            if realm != None and realm.last_updated_at != None:
                return service.find_by_modified_date_range(realm.last_updated_at)
            return service.find_by_modified_date_range()
        elif not obj:
...
class Realm(RealmMixin):
    ...
    last_updated_at = models.CharField(max_length=100,null=True)
